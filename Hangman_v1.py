...
def inputParsing(word_set_list, fails, letters_used, word_set):
    ...
    if try_letter in word:
        try_word = [word[i] if try_letter == word[i] else try_word[i]
...
while not inputValid:
    level_choice = input(
        "what level word do you want to play with?\n easy, medium, or hard: ")
...
visualize(word_sets[chosen_set[0]][chosen_word_num[0]][1], word_sets[chosen_set[0]]
          [chosen_word_num[0]][0], fails, letters_used, hint_sets[chosen_set[0]][chosen_word[0]])
...
while not gameover[0]:
    ...
    inputParsing(word_sets[chosen_set[0]], fails,
                 letters_used, chosen_word_num[0])
    ...
    reset_or_quit(word_sets[chosen_set[0]],
                  chosen_word_num[0], fails, gameover, retry, num_wins)
